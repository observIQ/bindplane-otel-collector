...
func TestBuildCaptureCommand_Darwin(t *testing.T) {
	tests := []struct {
		name     string
		iface    string
		filter   string
	...
	}{
		{
			...
			snaplen:  65535,
			promisc:  true,
			wantArgs: []string{"-i", "en0", "-n", "-xx", "-l", "--apple-md-print", "I", "-s", "65535"},
		},
		{
			...
			snaplen:  65535,
			promisc:  true,
			wantArgs: []string{"-i", "en0", "-n", "-xx", "-l", "--apple-md-print", "I", "-s", "65535", "tcp", "port", "443"},
		},
		{
			...
			snaplen:  1500,
			promisc:  true,
			wantArgs: []string{"-i", "en0", "-n", "-xx", "-l", "--apple-md-print", "I", "-s", "1500", "tcp", "port", "80", "or", "udp", "port", "53"},
		},
		{
			...
			snaplen:  65535,
			promisc:  false,
			wantArgs: []string{"-i", "en0", "-n", "-xx", "-l", "--apple-md-print", "I", "-p", "-s", "65535"},
		},
		{
			...
			snaplen:  1024,
			promisc:  true,
			wantArgs: []string{"-i", "en0", "-n", "-xx", "-l", "--apple-md-print", "I", "-s", "1024"},
		},
	}
...
func TestBuildCaptureCommand_Darwin_FilterWithParentheses(t *testing.T) {
	...
	// Verify macOS-specific flags are present
