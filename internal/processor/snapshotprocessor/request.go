...
// snapshotRequest specifies what snapshots to collect over the OpAMP
// custom-message channel. The on-the-wire format mirrors
// bindplane-otel-contrib/processor/snapshotprocessor's request schema
// so a single Bindplane server can serve both this processor and the
// contrib v2 processor.
type snapshotRequest struct {
	...
	// MaximumPayloadSizeBytes is the maximum size of the payload in bytes that should be sent.
