...
// New returns a new collector that uses the supplied factories. Callers
// (cmd/collector/main.go in the legacy build, internal/extension/opampconnectionextension/cmd/main/main.go in the
// ocb-driven build) own the factory set — either factories.DefaultFactories()
// or the ocb-generated components() function.
func New(configPaths []string, version string, loggingOpts []zap.Option, factories otelcol.Factories) Collector {
	return &collector{
