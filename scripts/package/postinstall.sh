...
install_sysconfig_service() {
  config_file="/etc/sysconfig/observiq-otel-collector"

  if [ ! -f "$config_file" ]; then
    echo "Installing sysconfig service to $config_file"
  else
    echo "Updating sysconfig service to $config_file"
  fi
