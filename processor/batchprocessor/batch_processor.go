...
type batch[T any] interface {
	...
	// splitBytes returns a full request built from pending items, splitting by byte size.
	splitBytes(sendBatchMaxSizeBytes int) (sentBatchSizeBytes int, req T)
	...
	// currentBytes returns the current accumulated size of the batch in bytes
	currentBytes() int
...
func newBatchProcessor[T any](set processor.Settings, cfg *Config, batchFunc func() batch[T]) (*batchProcessor[T], error) {
	...
	bp := &batchProcessor[T]{
		...
		sendBatchSizeBytes:    int(cfg.SendBatchSizeBytes),
		sendBatchMaxSizeBytes: int(cfg.SendBatchMaxSizeBytes),
		timeout:               cfg.Timeout,
		batchFunc:             batchFunc,
		shutdownC:             make(chan struct{}, 1),
	}
	// Choose batcher type based on configuration
	if len(cfg.BatchGroupByAttributes) > 0 {
		// Attribute-based grouping
		bp.batcher = newAttributeShardBatcher[T](bp, cfg.BatchGroupByAttributes, int(cfg.BatchGroupByAttributeLimit))
	} else if len(mks) == 0 {
		// Single shard (no grouping)
...
func (b *shard[T]) startLoop() {
	...
	if b.processor.timeout != 0 && (b.processor.sendBatchSize != 0 || b.processor.sendBatchSizeBytes != 0) {
...
func (b *shard[T]) processItem(item T) {
	...
	// Check if we should send based on item count or byte size
	for b.batch.itemCount() > 0 && (!b.hasTimer() || b.shouldSendBatch()) {
		sent = true
		// Determine trigger type based on which threshold was hit
		var trigger trigger
		if b.processor.sendBatchSizeBytes > 0 && b.batch.currentBytes() >= b.processor.sendBatchSizeBytes {
			trigger = triggerBatchSizeBytes
		} else {
			trigger = triggerBatchSize
		}
		b.sendItems(trigger)
	}
...
// shouldSendBatch checks if batch should be sent based on item count or byte size thresholds
func (b *shard[T]) shouldSendBatch() bool {
	// Check item count threshold
	if b.processor.sendBatchSize > 0 && b.batch.itemCount() >= b.processor.sendBatchSize {
		return true
	}
...
func (b *shard[T]) sendItems(trigger trigger) {
	...
	// Choose split method based on whether byte-based max size is configured
	if b.processor.sendBatchMaxSizeBytes > 0 {
		sent, req = b.batch.splitBytes(b.processor.sendBatchMaxSizeBytes)
	} else {
...
// attributeShardBatcher is used when batch_group_by_attributes is configured.
// It groups data by extracting attributes from the telemetry data itself.
type attributeShardBatcher[T any] struct {
	...
	lock          sync.Mutex
	size          int
	// extractFunc extracts attributes from the data
	extractFunc func(T, []string) attribute.Set
}

func newAttributeShardBatcher[T any](processor *batchProcessor[T], attributeKeys []string, attributeLimit int) *attributeShardBatcher[T] {
	ab := &attributeShardBatcher[T]{
		attributeKeys:  attributeKeys,
		attributeLimit: attributeLimit,
		processor:      processor,
	}
	
	// Set the appropriate extraction function based on type
	// We use a type assertion approach since Go generics don't support type switches
	var zero T
	switch any(zero).(type) {
	case ptrace.Traces:
		ab.extractFunc = func(data T, keys []string) attribute.Set {
			return extractAttributesFromTraces(any(data).(ptrace.Traces), keys)
		}
	case plog.Logs:
		ab.extractFunc = func(data T, keys []string) attribute.Set {
			return extractAttributesFromLogs(any(data).(plog.Logs), keys)
		}
	case pmetric.Metrics:
		ab.extractFunc = func(data T, keys []string) attribute.Set {
			return extractAttributesFromMetrics(any(data).(pmetric.Metrics), keys)
		}
	}
...
func (ab *attributeShardBatcher[T]) start(context.Context) error {
	return nil
}

func (ab *attributeShardBatcher[T]) consume(ctx context.Context, data T) error {
	// Extract attributes from the data itself
	aset := ab.extractFunc(data, ab.attributeKeys)
	
	b, ok := ab.batchers.Load(aset)
	if !ok {
		ab.lock.Lock()
		if ab.attributeLimit != 0 && ab.size >= ab.attributeLimit {
			ab.lock.Unlock()
			return errTooManyBatchers
		}
		
		var loaded bool
		b, loaded = ab.batchers.LoadOrStore(aset, ab.processor.newShard(nil))
		if !loaded {
			b.(*shard[T]).start()
			ab.size++
		}
...
func (ab *attributeShardBatcher[T]) currentMetadataCardinality() int {
	ab.lock.Lock()
...
func (bt *batchTraces) add(td ptrace.Traces) {
	...
	bt.byteCount += bt.sizer.TracesSize(td)
...
func (bt *batchTraces) split(sendBatchMaxSize int) (int, ptrace.Traces) {
	...
	if sendBatchMaxSize > 0 && bt.itemCount() > sendBatchMaxSize {
		...
		bt.byteCount = bt.sizer.TracesSize(bt.traceData)
...
func (bt *batchTraces) currentBytes() int {
	return bt.byteCount
}

func (bt *batchTraces) splitBytes(sendBatchMaxSizeBytes int) (int, ptrace.Traces) {
	var td ptrace.Traces
	var sentBytes int
	if sendBatchMaxSizeBytes > 0 && bt.currentBytes() > sendBatchMaxSizeBytes {
		// Split by estimating spans based on average byte size
		avgBytesPerSpan := bt.byteCount / bt.spanCount
		if avgBytesPerSpan == 0 {
			avgBytesPerSpan = 1
		}
		estimatedSpans := sendBatchMaxSizeBytes / avgBytesPerSpan
		if estimatedSpans == 0 {
			estimatedSpans = 1
		}
		td = splitTraces(estimatedSpans, bt.traceData)
		sentBytes = bt.sizer.TracesSize(td)
		bt.spanCount = bt.traceData.SpanCount()
		bt.byteCount = bt.sizer.TracesSize(bt.traceData)
	} else {
		td = bt.traceData
		sentBytes = bt.byteCount
		bt.traceData = ptrace.NewTraces()
		bt.spanCount = 0
		bt.byteCount = 0
	}
...
func (bm *batchMetrics) split(sendBatchMaxSize int) (int, pmetric.Metrics) {
	...
	if sendBatchMaxSize > 0 && bm.dataPointCount > sendBatchMaxSize {
		...
		bm.byteCount = bm.sizer.MetricsSize(bm.metricData)
		sent = sendBatchMaxSize
	} else {
		md = bm.metricData
...
func (bm *batchMetrics) currentBytes() int {
	return bm.byteCount
}

func (bm *batchMetrics) splitBytes(sendBatchMaxSizeBytes int) (int, pmetric.Metrics) {
	var md pmetric.Metrics
	var sentBytes int
	if sendBatchMaxSizeBytes > 0 && bm.currentBytes() > sendBatchMaxSizeBytes {
		// Split by estimating data points based on average byte size
		avgBytesPerDataPoint := bm.byteCount / bm.dataPointCount
		if avgBytesPerDataPoint == 0 {
			avgBytesPerDataPoint = 1
		}
		estimatedDataPoints := sendBatchMaxSizeBytes / avgBytesPerDataPoint
		if estimatedDataPoints == 0 {
			estimatedDataPoints = 1
		}
		md = splitMetrics(estimatedDataPoints, bm.metricData)
		sentBytes = bm.sizer.MetricsSize(md)
		bm.dataPointCount = bm.metricData.DataPointCount()
		bm.byteCount = bm.sizer.MetricsSize(bm.metricData)
	} else {
		md = bm.metricData
		sentBytes = bm.byteCount
		bm.metricData = pmetric.NewMetrics()
		bm.dataPointCount = 0
		bm.byteCount = 0
	}
...
func (bm *batchMetrics) add(md pmetric.Metrics) {
	...
	bm.byteCount += bm.sizer.MetricsSize(md)
...
func (bl *batchLogs) split(sendBatchMaxSize int) (int, plog.Logs) {
	...
	if sendBatchMaxSize > 0 && bl.logCount > sendBatchMaxSize {
		...
		bl.byteCount = bl.sizer.LogsSize(bl.logData)
		sent = sendBatchMaxSize
	} else {
		ld = bl.logData
		sent = bl.logCount
...
func (bl *batchLogs) currentBytes() int {
	return bl.byteCount
}

func (bl *batchLogs) splitBytes(sendBatchMaxSizeBytes int) (int, plog.Logs) {
	var ld plog.Logs
	var sentBytes int
	if sendBatchMaxSizeBytes > 0 && bl.currentBytes() > sendBatchMaxSizeBytes {
		// Split by estimating logs based on average byte size
		avgBytesPerLog := bl.byteCount / bl.logCount
		if avgBytesPerLog == 0 {
			avgBytesPerLog = 1
		}
		estimatedLogs := sendBatchMaxSizeBytes / avgBytesPerLog
		if estimatedLogs == 0 {
			estimatedLogs = 1
		}
		ld = splitLogs(estimatedLogs, bl.logData)
		sentBytes = bl.sizer.LogsSize(ld)
		bl.logCount = bl.logData.LogRecordCount()
		bl.byteCount = bl.sizer.LogsSize(bl.logData)
	} else {
		ld = bl.logData
		sentBytes = bl.byteCount
		bl.logData = plog.NewLogs()
		bl.logCount = 0
		bl.byteCount = 0
	}
...
func (bl *batchLogs) add(ld plog.Logs) {
	...
	bl.byteCount += bl.sizer.LogsSize(ld)
