...
// AgentDescription provides a mock function with no fields
...
// SetAvailableComponents provides a mock function with given fields: components
func (_m *MockOpAMPClient) SetAvailableComponents(components *protobufs.AvailableComponents) error {
	ret := _m.Called(components)

	if len(ret) == 0 {
		panic("no return value specified for SetAvailableComponents")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(*protobufs.AvailableComponents) error); ok {
		r0 = rf(components)
	} else {
		r0 = ret.Error(0)
	}
