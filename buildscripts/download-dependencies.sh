...
#
# Pass --aix to build only the AIX supervisor from the custom fork.
# Without --aix, builds all non-AIX platform supervisors from upstream.
set -e
...
mkdir -p "$DOWNLOAD_DIR/supervisor_bin"

if [ "$AIX_MODE" = true ]; then
